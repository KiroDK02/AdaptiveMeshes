...
public class TriangleFEQuadraticBaseWithNI :
    IFiniteElementWithNumericalIntegration<Vector2D>, ICalculatingMatrices, ISplittableElement
{
    public TriangleFEQuadraticBaseWithNI(string material, int[] vertexNumbers)
    {
        Material = material;
        ...
        Func<Vector2D, double> coefficient)
    {
        ...
        double LocalCoef(Vector2D vert) => GetCoefAtLocalCoords(vertexCoords, coefficient, vert);
    }
    ...
    {
        var edge = Edge(edgei);
        var vertex3 = GetThirdVertex(edge.i, edge.j);
    ...
    private double GetValueAtLocalPoint(ReadOnlySpan<double> weights, Vector2D localPoint)
    {
    ...
    private static int GetThirdVertex(int vertex1, int vertex2)
    {
        var vertex3 = vertex1 switch
        {
            0 => vertex2 switch
            {
                1 => 2,
