...
public class PardisoMatrix : IMatrix, IPardisoMatrix<double>
{
        ...
        for (int i = 0; i < dofs.Length; i++)
        {
            for (int j = i; j < dofs.Length; j++)
            {
                ...
                if (di <= dj)
                {
                    var i0 = ia[di];
                    var i1 = ia[di + 1];
                    var targetIndex = BinarySearch(ja, dj, i0, i1 - 1);
                ...
                else
                {
                    var i0 = ia[dj];
                    var i1 = ia[dj + 1];
                    var targetIndex = BinarySearch(ja, di, i0, i1 - 1);
    ...
    public void MultVect(ReadOnlySpan<double> to, double[] result)
    {
