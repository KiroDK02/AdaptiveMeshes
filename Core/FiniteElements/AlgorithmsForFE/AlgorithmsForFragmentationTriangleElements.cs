...
{
    public static IList<IFiniteElement> SplitToTriangles(IFiniteElement element, int[] globalVerticesNums,
        int countLayers)
    {
        ...
        for (int layer = 0; layer < countLayers; layer++)
        {
            ...
            for (int elemi = 0; elemi < amountElementsOnLayer; elemi++)
            {
                ...
                (int localNumV1, int localNumV2, int localNumV3) =
                    elemi % 2 == 0
                        ? (elemi / 2 + amountPassedVertices, elemi / 2 + amountPassedVertices + 1,
                            elemi / 2 + amountPassedVerticesForNextLayer)
                        : ((elemi + 1) / 2 + amountPassedVertices, (elemi + 1) / 2 + amountPassedVerticesForNextLayer,
                            (elemi + 1) / 2 + amountPassedVerticesForNextLayer - 1);
